...
def moving_average(data, window_size=5):
    ...
    for i in range(len(data)):
        ...
        clean_window = [v for v in window if isinstance(v, (int, float)) and not (isinstance(v, float) and v != v)]
        if clean_window:
...
# -------------------------
def save_csv(readings, temps_filtered, csv_path):
    """
    Guarda las lecturas en CSV incluyendo datos filtrados.
    - Se agregó la columna 'temp_filtered'.
    - Para poder comparar datos crudos vs filtrados.
    - No se pierde información original.
    - Facilita análisis posterior.
    """
    header = ["timestamp", "temp_raw", "temp_filtered", "ldr_raw"]
    lines = [",".join(header) + "\n"]
    ...
    for i, (ts, temp, ldr) in enumerate(readings):
        ...
        # NUEVO: valor filtrado
        temp_f = temps_filtered[i]
        temp_f_str = "" if (isinstance(temp_f, float) and temp_f != temp_f) else str(round(temp_f, 2))

        ldr_str = "" if ldr == -1 else str(ldr)

        lines.append(f"{ts},{temp_str},{temp_f_str},{ldr_str}\n")
...
# Graficado
# -------------------------
def plot_readings(readings, temps_filtered, plot_path):
    """
...
def main():
    ...
    # NUEVO: aplicar filtro de media móvil a la temperatura
    # Esto permite reducir el ruido y obtener una señal más estable
    temps_filtered = moving_average(temps, window_size=5)
    ...
    save_csv(readings, temps_filtered, csv_path)
    save_metadata(config, stats_temp, stats_ldr, metadata_path)

    plot_path = os.path.join(RESULTS_DIR, "plot.png")
    plot_readings(readings, temps_filtered, plot_path)

    umbral_alerta = 25.0
    picos_calor = [t for t in temps if t > umbral_alerta]
    alerta_msg = f"¡ALERTA! Se detectaron {len(picos_calor)} muestras por encima de {umbral_alerta}°C" if picos_calor else "Temperatura estable."

    print("\n--- Resumen v1.2.0 ---")
    print(f"Muestras: {len(readings)}")
    print(f"Estado: {alerta_msg}") # <--- Esta es la nueva línea
